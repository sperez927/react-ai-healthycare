...
| **v9-2** | **Backend commander-only task transitions** — `COMMANDER_ONLY_TRANSITIONS` constant in `Tasks::TransitionService`; `actor_role:` keyword param separates audit identity from role enforcement (actor is opaque email string); resolve, unblock, and reopen now require commander authority; `allowed_transitions` endpoint returns `commander_only: []` alongside `allowed` so clients never hardcode the list; `Task#prevent_resolved_at_change` relaxed to allow nil-clearing (required by `resolved_at_only_when_resolved` DB constraint when reopening); `TransitionService#call` clears `resolved_at` on non-resolved transitions; 19 new RSpec examples covering commander happy-paths, operator rejections, role-filtered allowed sets, and `commander_only` API field; 508 examples total |
| **v9-3** | **Frontend transition hygiene** — `MapPage` `TaskRow` replaced hardcoded `ALLOWED` transition table with `useAllowedTransitions(task.id)` hook; `TasksPage` ★ commander badge now driven from `commander_only` field in API response — `isCommanderOnlyTransition` function, `isCommander` guard, and dead `disabled` logic removed; `useAuth` import removed from `TasksPage`; operator factory trait `:operator` added for clean role-sensitive specs |

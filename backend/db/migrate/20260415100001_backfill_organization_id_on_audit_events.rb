class BackfillOrganizationIdOnAuditEvents < ActiveRecord::Migration[8.0]
  ...
  def up
    entity_type_sql = {
      ...
      "SignalRuleMatch" => "UPDATE audit_events SET organization_id = s.organization_id FROM signal_rule_matches srm JOIN sites s ON s.id = srm.site_id WHERE audit_events.entity_type = 'SignalRuleMatch' AND audit_events.entity_id = srm.id AND audit_events.organization_id IS NULL",
      "Asset" => "UPDATE audit_events SET organization_id = s.organization_id FROM assets a JOIN sites s ON s.id = a.home_site_id WHERE audit_events.entity_type = 'Asset' AND audit_events.entity_id = a.id AND audit_events.organization_id IS NULL",
      ...
      "Recommendation" => "UPDATE audit_events ae SET organization_id = sub.organization_id FROM (SELECT r.id, COALESCE(s.organization_id, ao.organization_id) AS organization_id FROM recommendations r LEFT JOIN sites s ON r.affected_entity_type = 'Site' AND s.id = r.affected_entity_id LEFT JOIN areas_of_operation ao ON r.affected_entity_type = 'AreaOfOperation' AND ao.id = r.affected_entity_id) sub WHERE ae.entity_type = 'Recommendation' AND ae.entity_id = sub.id AND ae.organization_id IS NULL AND sub.organization_id IS NOT NULL",
    }

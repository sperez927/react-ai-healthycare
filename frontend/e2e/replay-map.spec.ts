...
test('map replay smoke: login, enter replay, and show replay-safe map warnings', async ({ page }) => {
  ...
  await expect(page.getByText(/Historical AO overlays, risk shading, chokepoint overlays, geofence breach rings, and AIS vessel context remain available during replay\./i)).toBeVisible()
  await expect(page.getByText(/Live-only vessel enrichments remain limited\./i)).toBeVisible()
